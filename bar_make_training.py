...
# Time step for which training data will be generated
tstep = 3000

# Stencil size for contiguous Eulerian grid cells
nc = 3  # should be an odd number
nch = (nc - 1) // 2  # Integer division for Python

# Define output directory for training data
outdir = '.'

print("Reading parameters from params.txt...")
# Read parameters from params.txt - using a different approach since the file format is custom
...
rmin = params['rmin']
rmax = params['rmax']
rvec = (np.arange(nh) + 0.5) * (rmax - rmin) / nh  # Python arrays start at 0, not 1
...
# JIT-compiled function for processing grid data
@njit
def process_grid_data(nx, ny, nz, nh, grid, n3d, hist3d):
    ...
    for ix in range(nx):
        for iy in range(ny):
            for iz in range(nz):
                cnt += 1
                n3d[ix, iy, iz] = grid[cnt-1, 0]  # MATLAB is 1-indexed, so we adjust
                for ih in range(nh):
                    hist3d[ix, iy, iz, ih] = grid[cnt-1, ih+1]
    return n3d, hist3d

# Process grid data with JIT and progress bar
...
with tqdm(total=total_cells) as pbar:
    ...
    pbar.update(total_cells)  # Update outside since tqdm isn't Numba-compatible

print("Loading Eulerian data...")
# Load Eulerian data - MODIFIED to match the new file format (z,y,x order)
fdata = np.loadtxt(f'{outdir}/eul_m.txt')
...
# JIT-compiled function for processing Eulerian data
@njit
def process_eulerian_data(nx, ny, nz, fdata, sf, Tf, uf, vf, wf):
    ...
    for iz in range(nz):
        for iy in range(ny):
            for ix in range(nx):
                cnt += 1
                sf[iz, iy, ix] = fdata[cnt-1, 0]  # Adjusted for new loop order
...
# Process Eulerian data with JIT and progress bar
...
with tqdm(total=nz*ny*nx) as pbar:
    ...
    pbar.update(nz*ny*nx)  # Update outside since tqdm isn't Numba-compatible

print("Reading superdroplet data...")
# Read superdroplet data - custom approach to handle the file format
with open(f'{outdir}/lag_super.txt', 'r') as f:
    first_line = f.readline().strip()
    try:
        [float(val) for val in first_line.split()]
        headers = ['x', 'y', 'z', 'r', 's']  # Default column names
        data_start = 0
    except ValueError:
...
ns = len(tab['x'])  # Number of superdroplets
